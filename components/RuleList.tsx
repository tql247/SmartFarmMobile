...
export class RuleList extends Component<Props> {
      ...
      [
        // The "Yes" button
        {
          ...
          onPress: () => {
            ...
            const config = {
              method: "get",
              url: APIConfig["api"]["delete_rule"].replace("{_id}", _id),
              headers: {},
            };
        ...
        <View style={{ flexDirection: "row", justifyContent: "space-between" }}>
          ...
          >
            <TouchableOpacity
              ...
              onPress={() => this.editRule(item._id)}
            >
              <Text style={styles.title} numberOfLines={1}>
                {item.name}
              </Text>
              <Text>
                {item.machine.name} {item.target_value}
              </Text>
              {item.threshold !== null && (
                <Text>
                  {item.sensor.name} {item.expr} {item.threshold}
                </Text>
              )}
              <Text>
  ...
  renderItem(item: any) {
    return <View style={styles.imgContainer}>{this.renderTitle(item)}</View>;
  }
    ...
    return (
      <View style={styles.flatListContainer}>
        <Picker
          ...
          onValueChange={(itemValue) => this.filterValueByFarm(itemValue)}
          style={styles.picker}
          itemStyle={{ height: 44 }}
        >
        ...
        <FlatList
          ...
          renderItem={({ item }) => (
            <View style={styles.imgContainer}>{this.renderItem(item)}</View>
          )}
        />
      </View>
    );
